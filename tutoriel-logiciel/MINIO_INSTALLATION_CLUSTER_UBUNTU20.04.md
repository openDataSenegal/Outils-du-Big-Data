...
Il faut définir les variables d'environnement dans le fichier minio qui sera créé dans le repertoire /etc/default qui seront utilisées par systemd:
```
    MINIO_ROOT_USER=user_name_compte_admin
    MINIO_ROOT_PASSWORD=mot_de_pass_compte_admin
    MINIO_VOLUMES="https://minio{1..2}.panongbene.com:9000/path_ou_les_donnes_seront_stocker"
    MINIO_OPTS="--console-address :9001"
    MINIO_SERVER_URL="http://adress_ip_1:9000"
```
## Démarage de MinIO
...
Créer le repertoire .minio/certs dans $HOME
...
Une fois les certificats https générés, il faut changer la configuration de la variable d'environnement MINIO_OPTS dans le fichier /etc/default/minio:
```
   MINIO_OPTS="--certs-dir /chemin_vers_les_certs/.minio/certs --console-address :9001"
```
...
Pour la configuration du Noeud 2, il faut commencer par assigner un nom de domaine et de configurer le DNS de la machine.
Nous allons assigner le nom de domaine minio1.panongbene.com à la machine1 et minio2.panongbene.com à la machine 2.
Pour configurer le DNS, il faut éditer le fichier /etc/hosts. Pour cela, executer la commande suivante:
```
  nano /etc/hosts
...
commande ci-dessous.
```
  ping -c 4 minio2.panongbene.com
```
...
```
    ...
    MINIO_VOLUMES="https://minio{1..2}.panongbene.com:9000/path_ou_les_donnes_seront_stocker"
    MINIO_OPTS="--console-address :9001"
    MINIO_SERVER_URL="http://adress_ip_2:9000"
```
...
Il faut ensuite générer les certificats avec les commandes ci-dessous:
```
   sudo certgen -host minio1.panongbene.com,adress_ip_2
```
Il faut ensuite créer le repertoire .minio/certs/minio1.panongbene.com qui contiendra les certificats (private.key et public.crt) générés dans la premiere machine.
Il faut aussi dans la machine1 créer le repertoire .minio/certs/minio2.panongbene.com qui contiendra les certificats (private.key et public.crt) générés dans la deuxieme machine.
N'oublier de changer les droits en executant les commandes ci-dessous:
```
   chmod +x private.key
   chmod +x public.crt
```
...
```
   MINIO_OPTS="--certs-dir /chemin_vers_les_certs/.minio/certs --console-address :9001"
```
On peut alors relancer le MinIO des deux machines en utilisant les commandes ci-dessous.
